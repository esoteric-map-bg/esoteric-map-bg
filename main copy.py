...
# ==============================================================================
# 4. УМНАТА ФОРМА (С ВЪЗДУХ ОТДОЛУ)
# ==============================================================================
def render_main_user_smart_form(key_context, is_compatibility=False):
    ...
    # ТУК Е МАГИЯТА ЗА БУТОНА - 300px ВЪЗДУХ ОТДОЛУ
...
# ==============================================================================
def run_desktop():
    # CSS ЗА PC - ПОПРАВКА НА СТРЕЛКАТА
    st.markdown(f"""
    <style>
        .block-container {{ padding: 0 !important; max-width: 100% !important; }}
        header {{ display: none !important; }}
        
        /* НАСИЛСТВЕНО ПОКАЗВАНЕ НА СТРЕЛКАТА ЗА АДМИНА */
        ...
        /* МАНДАЛА ЦЕНТРИРАНЕ - ОЩЕ ПО-НАДОЛУ */
        ...
        .ellipse-node {{ position: absolute; width: 190px; height: 55px; border-radius: 50%; display: flex; justify-content: center; align-items: center; color: white !important; font-weight: bold; font-size: 12px; z-index: 200; border: 2px solid rgba(255,255,255,0.9); backdrop-filter: blur(5px); }}
        ...
        .hero-back-btn {{ position: absolute; top: 60px; left: 20px; z-index: 999; text-decoration: none !important; background: rgba(0,0,0,0.6); border: 1px solid white; color: #fff; padding: 8px 20px; border-radius: 8px; font-weight: bold; }}
    ...
    with st.sidebar:
        ...
        st.text_input("User"); st.text_input("Pass", type="password"); st.button("LOGIN")

    if st.session_state['current_view'] == 'HOME':
        # ГОЛЯМА МАНДАЛА - СВАЛЕНА НА 410px
        ...
        c_x, c_y = 500, 410 # ТОВА Е НОВИЯТ ЦЕНТЪР
        r_x, r_y = 350, 200
        ...
        if not st.session_state['start_horoscope']:
            ...
            st.markdown('<a href="?view=HOME&device=desktop" target="_self" class="hero-back-btn">⬅️ НАЗАД</a>', unsafe_allow_html=True)
            try:
            ...
            if st.session_state['western_cat'] is None:
                if st.button("⬅️ НАЗАД", key="pc_back2"): st.session_state['start_horoscope'] = False; st.rerun()
                ...
                with c_back:
                    if st.button("⬅️ НАЗАД", key="pc_back3"): st.session_state['western_cat'] = None; st.rerun()
...
def run_mobile():
    ...
    <style>
        ...
        .mob-container {{ position: relative; width: 360px; height: 500px; margin: 20px auto; }}
        .mob-sun {{ position: absolute; width: 100px; height: 100px; border-radius: 50%; background: radial-gradient(circle, #FFD700 0%, #FF8C00 100%); left: 130px; top: 180px; display: flex; justify-content: center; align-items: center; border: 4px solid white; box-shadow: 0 0 20px #FFD700; z-index: 10; color: black !important; font-weight: bold; font-size: 11px; text-align: center; }}
        ...
        .hero-back-btn {{ position: absolute; top: 20px; left: 20px; z-index: 999; text-decoration: none !important; background: rgba(0,0,0,0.6); border: 1px solid white; color: #fff; padding: 8px 20px; border-radius: 8px; font-weight: bold; }}
    ...
    if st.session_state['current_view'] == 'HOME':
        ...
        mh.append('<svg width="360" height="500" style="position: absolute; top:0; left:0; z-index:1;">')
        mx, my = 155, 175
        ...
        if not st.session_state['start_horoscope']:
            ...
            if st.session_state['western_cat'] is None:
                ...
                # ИЗПОЛЗВАМЕ СЪЩАТА УМНА ФОРМА, НО ТЯ ЩЕ СЕ СВИЕ АВТОМАТИЧНО

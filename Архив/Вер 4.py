...
def render_date_selectors(key_prefix, mode="birth"):
    ...
    years_list = YEARS_BIRTH if mode == "birth" else YEARS_FUTURE if mode == "future" else YEARS_HISTORY
...
def render_person_form(key_suffix, show_header=True):
    if show_header: st.subheader("Данни")
    c1, c2 = st.columns([2, 1])
    with c1: name = st.text_input("Име:", key=f"name_{key_suffix}")
    with c2: gender = st.radio("Пол:", ["👩 Жена", "👨 Мъж"], horizontal=True, key=f"gender_{key_suffix}")
    st.write("Дата на раждане:")
    d, m, y = render_date_selectors(f"birth_{key_suffix}")
...
if "view" in params:
    ...
    if 'keep_form' not in st.session_state:
        st.session_state['show_form'] = False
...
<style>
    ...
    .ellipse-node:hover {{ 
        transform: scale(1.1); box-shadow: 0 0 30px gold; 
    }}
    ...
    .intro-box {{
        ...
        margin-bottom: 30px; color: #f0f0f0 !important;
    }}
</style>
...
with st.sidebar:
    if st.session_state['current_view'] == 'HOME':
        ...
        st.markdown("<p style='color: #FFD700; text-align:center; font-weight:bold;'>БЪРЗ ДОСТЪП</p>", unsafe_allow_html=True)
        
        for m in modules:
            st.markdown(f"""
            <a href="?view={m['key']}" target="_self" style="text-decoration:none;">
                <div style="
                    background: {m['color']}; padding: 5px; margin-bottom: 4px; 
    ...
    if key == "Western":
        if st.session_state.get('start_horoscope', False):
            ...
            <style>
                ...
                .block-container { max-width: 100% !important; padding: 2rem 5rem !important; }
            </style>
            """, unsafe_allow_html=True)
            ...
            if st.session_state['astro_section'] == 'menu':
                ...
                st.markdown('<div class="intro-box">"Вселената е часовников механизъм. Планетите никога не спират да се движат. Тук ще видиш как текущото им положение активира твоята карта."</div>', unsafe_allow_html=True)
                type_forecast = st.radio("Избери тип:", ["👤 Лична прогноза", "💑 Прогноза за ДВАМА", "🎂 Солар"], horizontal=True)
                st.write("---")
                ...
                render_date_selectors("forecast_start", mode="history")
                ...
                st.markdown('<div class="intro-box">"Всяко начало има свое бъдеще. Искаш успех в бизнеса или щастие в брака? Използвай древната мъдрост на Елекционната астрология."</div>', unsafe_allow_html=True)
                ...
                with c1:
                    ...
                    st.selectbox("Цел:", ["💼 Бизнес", "💍 Сватба", "✈️ Пътуване", "🏥 Здраве", "🏠 Имот"])
                with c2:
                    st.subheader("3. Начална дата за търсене:")
                    render_date_selectors("elec_start", mode="future")
                    st.selectbox("Период:", ["1 седмица", "1 месец", "3 месеца"])
            ...
            <style>
                ...
                div.stButton > button { position: relative !important; z-index: 99999 !important; top: -80px !important; }
            </style>
            """, unsafe_allow_html=True)
            ...
            try:
                ...
                st.error("Грешка при зареждане на изображението")
        ...
        st.markdown(f"""<div style="text-align: center;"><h1 style="color:{data['color']}; text-shadow: 0 0 25px {data['color']};">{data['name']}</h1></div>""", unsafe_allow_html=True)
        st.markdown(f"""<p style='text-align:center; font-size:1.2rem;'>Модулът <b>{data['name']}</b> е в процес на разработка...</p>""", unsafe_allow_html=True)
